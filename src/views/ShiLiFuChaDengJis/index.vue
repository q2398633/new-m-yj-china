<template>
  <div class="Child_Box">
          ...
          <van-checkbox-group v-model="result" @change="CheckBoxChange">
            ...
            >
              ...
              <div class="ChildName" @click="Child_Countend($event)">
                <div>{{ Child.foodName }}</div>
                <div class="ChildName_Bottom2" style="margin-left: 0;">
                  {{ Child.lingYongRen }}
                </div>
                <div class="ChildName_Bottom2">{{ Child.jiaGongRen }}</div>
                <div class="ChildName_Bottom2">{{ Child.chuLiFangFa }}</div>
                <div class="ChildName_Bottom2">{{ Child.jiaGongDate }}</div>
              </div>
            </van-cell>
          </van-checkbox-group>
        </div>
    ...
    >
      ...
      <div style="margin-top: 2.5rem">
        ...
        <van-form @submit="onSubmit">
          <van-field
            v-model="ChildListSearch.foodName"
            label="食材名称"
            clearable
            placeholder="食材名称"
          />
          <van-field
            v-model="ChildListSearch.lingYongRen"
            name="lingYongRen"
            label="领用人"
            placeholder="领用人"
          />
          ...
          />
          <van-field
            v-model="ChildListSearch.chuLiFangFa"
            name="chuLiFangFa"
            label="处理方式"
            placeholder="处理方式"
          />
          <van-field
            v-model="ChildListSearch.chuLiShuLiang"
            name="chuLiShuLiang"
            label="处理数量"
            placeholder="处理数量"
          />
          <van-field
            v-model="ChildListSearch.keYongShuLiang"
            name="keYongShuLiang"
            label="可用数量"
            placeholder="可用数量"
          />
          <van-field
            v-model="ChildListSearch.jiaGongDate"
            name="jiaGongDate"
            label="加工日期"
            placeholder="加工日期"
            @focus="noBomBox"
    ...
    <van-popup v-model="ShowBirthday" round position="bottom">
      <van-datetime-picker
        :value-class="className"
        :value="ChildListSearch.jiaGongDate"
        type="date"
        title="加工日期"
...
import { SLFCList, SLFCDelete } from "@/api/ShiLiFuChaDengJis";
...
export default {
  ...
  methods: {
    async onLoad() {
      ...
      this.LoadPage.page++;
      var that = this;
      const { data } = await SLFCList(this.LoadPage);
      var ChildList = data.data;
      if (data.code == 200) {
        for (var i in ChildList) {
          if (that.ChildName.length < ChildList.length) {
            that.ChildName.push({
              foodName: ChildList[i].foodName,
              lingYongRen: ChildList[i].lingYongRen,
              jiaGongRen: ChildList[i].jiaGongRen,
              jiaGongDate: ChildList[i].jiaGongDate,
              chuLiFangFa: ChildList[i].chuLiFangFa,
              chuLiShuLiang: ChildList[i].chuLiShuLiang,
              keYongShuLiang: ChildList[i].keYongShuLiang,
    ...
    async onSubmit(values) {
      // 筛选赋值
      this.ChildListSearch.chuLiShuLiang =
        this.ChildListSearch.chuLiShuLiang - 0;
      this.ChildListSearch.keYongShuLiang =
        this.ChildListSearch.keYongShuLiang - 0;
      const { data } = await SLFCList(this.ChildListSearch);
    ...
    BirthdayConfirm(val) {
      ...
      this.ChildListSearch.jiaGongDate = `${year}-${month}-${day}`;
    ...
    async tabClick(event) {
      ...
      if (ButtonText === "添加") {
        if (this.ckindex > 0) {
          ...
          this.$router.replace("/ShiCaiChuLiJiLusAdd");
        }
        this.CheckboxIndex = [];
      } else if (ButtonText === "修改") {
        if (this.ckindex <= 1) {
          this.$router.push({
            name: "ShiCaiChuLiJiLusEdit",
            params: {
              foodName: this.CheckboxID.foodName,
              lingYongRen: this.CheckboxID.lingYongRen,
              jiaGongRen: this.CheckboxID.jiaGongRen,
              jiaGongDate: this.CheckboxID.jiaGongDate,
              chuLiFangFa: this.CheckboxID.chuLiFangFa,
              chuLiShuLiang: this.CheckboxID.chuLiShuLiang,
              keYongShuLiang: this.CheckboxID.keYongShuLiang,
        ...
        if (this.ckindex <= 1) {
          ...
          const { data } = await SLFCDelete(this.CID);
          if (data.code == 200) {
    ...
    Child_Countend(event) {
      for (var i in this.$refs.checkboxes) {
        if (event.target.innerText == this.$refs.checkboxes[i].name.foodName) {
          this.$router.push({
            name: "ShiCaiChuLiJiLusDetail",
      ...
      for (var i = 0; i < MenuList.length; i++) {
        if (MenuList[i].item.name == "食堂管理") {
          ...
          for (var q = 0; q < MenuList[i].children.length; q++) {
            if (MenuList[i].children[q].item.name == "食材处理记录") {
              for (
                var w = 0;
