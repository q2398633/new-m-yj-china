            ...
            >
              ...
              <div class="ChildName" @click="Child_Countend($event)">
                ...
                <div class="ChildName_Bottom2">
                  医院名称: {{ Child.yiYuanMingCheng }}
                </div>
                <div class="ChildName_Bottom2">左眼: {{ Child.zuoYan }}°C</div>
                <div class="ChildName_Bottom2">右眼: {{ Child.youYan }}°C</div>
              </div>
            </van-cell>
          </van-checkbox-group>
        </div>
    ...
    >
      ...
      <div style="margin-top: 1.5rem">
        ...
        <van-form @submit="onSubmit">
          ...
          />
          <van-field
            v-model="ChildListSearch.banJi"
            name="banJi"
            label="班级"
            placeholder="班级"
          />
          <van-field
            v-model="ChildListSearch.xingBie"
            clearable
            name="xingBie"
            label="性别"
            placeholder="性别"
          ...
          />
          <van-field
            v-model="ChildListSearch.yiYuanMingCheng"
            name="yiYuanMingCheng"
            label="医院名称"
            placeholder="医院名称"
          />
          <van-field
            v-model="ChildListSearch.zuoYan"
            name="zuoYan"
            label="左眼"
            placeholder="左眼"
          />
          <van-field
            v-model="ChildListSearch.youYan"
            name="youYan"
            label="右眼"
            placeholder="右眼"
          />
          <van-field
            v-model="ChildListSearch.queZhenMingCheng"
            name="queZhenMingCheng"
            label="确诊名称"
            placeholder="确诊名称"
          />
          <van-field
            v-model="ChildListSearch.jiaoZhiFangFa"
            name="jiaoZhiFangFa"
            label="矫治方式"
            placeholder="矫治方式"
          />
...
import { SLDJList, SLDJDelete } from "@/api/ShiLiDengJis";
...
export default {
  ...
  methods: {
    async onLoad() {
      ...
      this.LoadPage.page++;
      var that = this;
      const { data } = await SLDJList(this.LoadPage);
        ...
        for (var i in ChildList) {
          if (that.ChildName.length < ChildList.length) {
            that.ChildName.push({
              ...
              yiYuanMingCheng: ChildList[i].yiYuanMingCheng,
              zuoYan: ChildList[i].zuoYan,
              youYan: ChildList[i].youYan,
              queZhenMingCheng: ChildList[i].queZhenMingCheng,
              jiaoZhiFangFa: ChildList[i].jiaoZhiFangFa,
    ...
    async onSubmit(values) {
      // 筛选赋值
      this.ChildListSearch.xingBie = this.ChildListSearch.xingBie - 0;
      this.ChildListSearch.nianLing = this.ChildListSearch.nianLing - 0;
      this.ChildListSearch.zuoYan = this.ChildListSearch.zuoYan - 0;
      this.ChildListSearch.youYan = this.ChildListSearch.youYan - 0;
      const { data } = await SLDJList(this.ChildListSearch);
    ...
    async tabClick(event) {
      ...
      if (ButtonText === "添加") {
        if (this.ckindex > 0) {
          ...
          this.$router.replace("/ShiLiDengJisAdd");
        }
        this.CheckboxIndex = [];
      } else if (ButtonText === "修改") {
        if (this.ckindex <= 1) {
          this.$router.push({
            name: "ShiLiDengJisEdit",
            params: {
              ...
              yiYuanMingCheng: this.CheckboxID.yiYuanMingCheng,
              zuoYan: this.CheckboxID.zuoYan,
              youYan: this.CheckboxID.youYan,
              queZhenMingCheng: this.CheckboxID.queZhenMingCheng,
              jiaoZhiFangFa: this.CheckboxID.jiaoZhiFangFa,
        ...
        if (this.ckindex <= 1) {
          ...
          const { data } = await SLDJDelete(this.CID);
          if (data.code == 200) {
      ...
      for (var i = 0; i < MenuList.length; i++) {
        if (MenuList[i].item.name == "日常工作表") {
          ...
          for (var q = 0; q < MenuList[i].children.length; q++) {
            if (MenuList[i].children[q].item.name == "视力矫治登记") {
              for (
                var w = 0;
