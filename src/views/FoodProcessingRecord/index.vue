  ...
  <div>
    <div class="parentAdmin">
      ...
      <van-popup v-model="show"
                 ...
                 close-icon="close">
        <van-cell-group>
          <div style="font-size: 30px; width: 96.2%; height: 53px; line-height: 53px; color: white; font-family: '楷体'; background: #0199ff; padding-left:15px;">搜索食材处理记录</div>
          <van-field label="食材:"
                     label-width="70px"
                     autosize
          ...
          <van-list v-model="loading"
            ...
            <van-cell v-for="(item) in list"
              ...
              <van-swipe-cell style="border: 7px solid rgb(231, 231, 231);">
                <div class="head">
                  <img src="../../assets/SCCL.jpg"
                       alt="">
                  <h1 style="height: 1rem;color: black; font-size: .5rem;line-height: 1rem;font-weight: 700; font-family: '楷体'; margin-left: 45px;">{{ item.FoodName }}</h1>
                </div>
                ...
                <van-cell :border="false"
                          title="加工日期:"
                          style="padding-left:30px; padding-right: 30px;">
                  {{ item.JiaGongDate | dateFmt('YYYY-MM-DD') }}
                </van-cell>
                <template slot="right">
                  <van-button square
                              type="danger"
      ...
      <van-button type="info"
                  ...
                  @click.prevent="AddList">添加食材处理记录</van-button>
...
<script>
import { FoodProcessingRecord } from '@/api/FoodProcessingRecord'
...
export default {
  ...
  created () {
    // 页面一进入加载食材处理记录表
    this.loadFoodProcessingRecordList()
  },
  methods: {
    back () {
      this.$router.go(-1)
    ...
    // 晨午晚检
    async loadFoodProcessingRecordList () {
      let channels = []
      const data = await FoodProcessingRecord()
    ...
    async onLoad () {
      const data = await this.loadFoodProcessingRecordList()
      this.list = data
      this.isLoading = false
      this.loading = false
      this.finished = true
