      ...
      <van-pull-refresh v-model="isLoading"
        ...
        <van-list v-model="loading"
          ...
          <van-cell v-for="(item) in list"
            ...
            <span> {{ '类别: '+item.WeiHuType }}</span>
            <span> {{ '经手人: ' +item.WeiHuRen }}</span>
            <span> {{ '费用金额: ' +item.FeiYong }}</span>
            <span> {{ '维护内容: ' +item.WeiHuNeiRong }}</span>
            <span> {{ '备注: ' +item.BeiZhu }}</span>
            <span> {{ '维护日期: ' +item.Date }}</span>
          </van-cell>
        </van-list>
      </van-pull-refresh>
      <!-- 分页 -->
...
export default {
    ...
    methods: {
        ...
        async onLoad () {
            const data = await this.loadAssetMaintenance()
            this.list = data
        }
    }
}
