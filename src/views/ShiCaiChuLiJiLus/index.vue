...
import { SCCLList, SCCLDelete } from "@/api/ShiCaiChuLiJiLus";
...
export default {
  ...
  methods: {
    async onLoad() {
      ...
      this.LoadPage.page++;
      var that = this;
      const { data } = await SCCLList(this.LoadPage);
    ...
    async onSubmit(values) {
      ...
      const { data } = await SCCLList(this.ChildListSearch);
    ...
    async tabClick(event) {
      ...
      if (ButtonText === "添加") {
        ...
        if (this.ckindex <= 1) {
          ...
          const { data } = await SCCLDelete(this.CID);
          if (data.code == 200) {
