  ...
  <div class="Child_Box">
          ...
          <van-checkbox-group v-model="result" @change="CheckBoxChange">
            ...
            >
              ...
              <div class="ChildName" @click="Child_Countend($event)">
                <div>{{ Child.caiYao }}</div>
                <div class="ChildName_Bottom">{{ Child.canBie }}</div>
                <div class="ChildName_Bottom2">{{ Child.jiaGongRen }}</div>
                <div class="ChildName_Bottom2">{{ Child.liuYangRen }}</div>
              </div>
            </van-cell>
          </van-checkbox-group>
        </div>
    ...
    >
      ...
      <div style="margin-top: 2.5rem">
        ...
        <van-form>
          <van-field
            v-model="ChildListSearch.caiYao"
            label="菜肴"
            clearable
            @change="NameChange"
            placeholder="菜肴"
          ...
          />
          <van-field
            v-model="ChildListSearch.jiaGongRen"
            clearable
            name="jiaGongRen"
            label="加工人"
            placeholder="加工人"
          />
          <van-field
            v-model="ChildListSearch.liuYangRen"
            name="liuYangRen"
            label="留样人"
            placeholder="留样人"
          />
          <van-field
            v-model="ChildListSearch.liuYangRiQi"
            name="liuYangRiQi"
            label="留样日期"
            placeholder="留样日期"
            @focus="noBomBox"
            :value-class="LiuYangName"
            :value="ChildListSearch.liuYangRiQi"
            @click="ShowBirthday = true"
          />
          <van-field
            v-model="ChildListSearch.liuYangWeiZhi"
            name="liuYangWeiZhi"
            label="留样位置"
            placeholder="留样位置"
          />
          <van-field
            v-model="ChildListSearch.xiaoHuiRen"
            name="xiaoHuiRen"
            label="销毁人"
            placeholder="销毁人"
          />
          <van-field
            v-model="ChildListSearch.beiZhu"
            name="beiZhu"
            label="备注"
            placeholder="备注"
          />
          <van-field
            v-model="ChildListSearch.yuJiXiaoHuiRiQi"
          ...
          />
          <van-field
            v-model="ChildListSearch.xiaoHuiRiQi"
            name="xiaoHuiRiQi"
            label="销毁日期"
            placeholder="销毁日期"
    ...
    <van-popup v-model="ShowYJXHDate" round position="bottom">
      <van-datetime-picker
        ...
        :value="ChildListSearch.yuJiXiaoHuiRiQi"
        type="date"
        title="预计销毁日期"
    ...
    <van-popup v-model="ShowXHDate" round position="bottom">
      <van-datetime-picker
        :value-class="XHName"
        :value="ChildListSearch.XiaoHuiRiQi"
        type="date"
        title="销毁日期"
...
import { CPLYList, CPLYDelete } from "@/api/CaiPinLiuYangs";
...
export default {
  ...
  methods: {
    async onLoad() {
      ...
      this.LoadPage.page++;
      var that = this;
      const { data } = await CPLYList(this.LoadPage);
      var ChildList = data.data;
      if (data.code == 200) {
        for (var i in ChildList) {
          if (that.ChildName.length < ChildList.length) {
            that.ChildName.push({
              beiZhu: ChildList[i].beiZhu,
              canBie: ChildList[i].canBie,
              caiYao: ChildList[i].caiYao,
              jiaGongRen: ChildList[i].jiaGongRen,
              liuYangRiQi: ChildList[i].liuYangRiQi,
              liuYangRen: ChildList[i].liuYangRen,
              liuYangWeiZhi: ChildList[i].liuYangWeiZhi,
              yuJiXiaoHuiRiQi: ChildList[i].yuJiXiaoHuiRiQi,
              xiaoHuiRiQi: ChildList[i].xiaoHuiRiQi,
              xiaoHuiRen: ChildList[i].xiaoHuiRen,
      ...
      for (var s in this.ChildName) {
        if (this.ChildName[s].canBie == 1) {
          this.ChildName[s].canBie = "早点";
        } else if (this.ChildName[s].canBie == 0) {
          this.ChildName[s].canBie = "早餐";
        } else if (this.ChildName[s].canBie == 2) {
          this.ChildName[s].canBie = "午餐";
        } else if (this.ChildName[s].canBie == 3) {
          this.ChildName[s].canBie = "午点";
        } else if (this.ChildName[s].canBie == 4) {
          this.ChildName[s].canBie = "晚餐";
        } else if (this.ChildName[s].canBie == 5) {
          this.ChildName[s].canBie = "晚点";
        }
      }
    },
    ...
    async onSubmit() {
      // 筛选赋值
      this.see = false;
      const { data } = await CPLYList(this.ChildListSearch);
    ...
    onConfirm4(value) {
      if (value == "早餐") {
        this.ChildListSearch.canBie = 0 - 0;
      } else if (value == "早点") {
        this.ChildListSearch.canBie = 1 - 0;
      } else if (value == "午餐") {
        this.ChildListSearch.canBie = 2 - 0;
      } else if (value == "午点") {
        this.ChildListSearch.canBie = 3 - 0;
      } else if (value == "晚餐") {
        this.ChildListSearch.canBie = 4 - 0;
      } else if (value == "晚点") {
        this.ChildListSearch.canBie = 5 - 0;
      }
      this.CanBieXZ = value;
    ...
    BirthdayConfirm(val) {
      ...
      this.ChildListSearch.liuYangRiQi = `${year}-${month}-${day}`;
    ...
    Rdate(val) {
      ...
      this.ChildListSearch.yuJiXiaoHuiRiQi = `${year}-${month}-${day}`;
    ...
    XHdate(val) {
      ...
      this.XHName = "销毁日期";
      this.ChildListSearch.xiaoHuiRiQi = `${year}-${month}-${day}`;
    ...
    async tabClick(event) {
      ...
      if (ButtonText === "添加") {
        if (this.ckindex > 0) {
          ...
          this.$router.replace("/CaiPinLiuYangsAdd");
        }
        this.CheckboxIndex = [];
      } else if (ButtonText === "修改") {
        if (this.ckindex <= 1) {
          this.$router.push({
            name: "CaiPinLiuYangsEdit",
            params: {
              beiZhu: this.CheckboxID.beiZhu,
              canBie: this.CheckboxID.canBie,
              caiYao: this.CheckboxID.caiYao,
              jiaGongRen: this.CheckboxID.jiaGongRen,
              id: this.CheckboxID.id,
              liuYangRiQi: this.CheckboxID.liuYangRiQi,
              liuYangRen: this.CheckboxID.liuYangRen,
              liuYangWeiZhi: this.CheckboxID.liuYangWeiZhi,
              yuJiXiaoHuiRiQi: this.CheckboxID.yuJiXiaoHuiRiQi,
              xiaoHuiRiQi: this.CheckboxID.xiaoHuiRiQi,
              xiaoHuiRen: this.CheckboxID.xiaoHuiRen,
        ...
        if (this.ckindex <= 1) {
          ...
          const { data } = await CPLYDelete(this.CID);
          if (data.code == 200) {
    ...
    Child_Countend(event) {
      for (var i in this.$refs.checkboxes) {
        if (event.target.innerText == this.$refs.checkboxes[i].name.caiYao) {
          this.$router.push({
            name: "CaiPinLiuYangsDetail",
    ...
    async PickerJson() {
      ...
      for (var i = 0; i < MenuList.length; i++) {
        if (MenuList[i].item.name == "食堂管理") {
          for (var q = 0; q < MenuList[i].children.length; q++) {
            if (MenuList[i].children[q].item.name == "菜品留样") {
              for (
                var w = 0;
