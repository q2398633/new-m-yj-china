  ...
  <div class="Child_Box">
          ...
          <van-checkbox-group v-model="result" @change="CheckBoxChange">
            ...
            >
              ...
              <div class="ChildName" @click="Child_Countend($event)">
                <div>{{ Child.xiaoDuFangFa }}</div>
                <div class="ChildName_Bottom">{{ Child.canBie }}</div>
                <div class="ChildName_Bottom2">{{ Child.caoZuoYuan }}</div>
                <div class="ChildName_Bottom2">{{ Child.fuZeRen }}</div>
    ...
    >
      ...
      <div style="margin-top: 2.5rem">
        ...
        <van-form>
          <van-field
            v-model="ChildListSearch.xiaoDuFangFa"
            label="消毒方式"
            clearable
            @change="NameChange"
            placeholder="消毒方式"
          ...
          />
          <van-field
            v-model="ChildListSearch.caoZuoYuan"
            clearable
            name="caoZuoYuan"
            label="操作员"
            placeholder="操作员"
          />
          <van-field
            v-model="ChildListSearch.fuZeRen"
            name="fuZeRen"
            label="负责人"
            placeholder="负责人"
          />
...
import { CJXDList, CJXDDelete } from "@/api/CanJuXiaoDus";
...
export default {
  ...
  methods: {
    async onLoad() {
      ...
      this.LoadPage.page++;
      var that = this;
      const { data } = await CJXDList(this.LoadPage);
      var ChildList = data.data;
      if (data.code == 200) {
        for (var i in ChildList) {
          if (that.ChildName.length < ChildList.length) {
            that.ChildName.push({
              fuZeRen: ChildList[i].fuZeRen,
              canBie: ChildList[i].canBie,
              date: ChildList[i].date,
              caoZuoYuan: ChildList[i].caoZuoYuan,
              xiaoDuFangFa: ChildList[i].xiaoDuFangFa,
    ...
    async onSubmit() {
      // 筛选赋值
      this.see = false;
      const { data } = await CJXDList(this.ChildListSearch);
    ...
    async tabClick(event) {
      ...
      if (ButtonText === "添加") {
        if (this.ckindex > 0) {
          ...
          this.$router.replace("/CanJuXiaoDusAdd");
        }
        this.CheckboxIndex = [];
      } else if (ButtonText === "修改") {
        if (this.ckindex <= 1) {
          this.$router.push({
            name: "CanJuXiaoDusEdit",
            params: {
              fuZeRen: this.CheckboxID.fuZeRen,
              canBie: this.CheckboxID.canBie,
              date: this.CheckboxID.date,
              caoZuoYuan: this.CheckboxID.caoZuoYuan,
              xiaoDuFangFa: this.CheckboxID.xiaoDuFangFa,
        ...
        if (this.ckindex <= 1) {
          ...
          const { data } = await CJXDDelete(this.CID);
          if (data.code == 200) {
      ...
      for (var i = 0; i < MenuList.length; i++) {
        if (MenuList[i].item.name == "食堂管理") {
          for (var q = 0; q < MenuList[i].children.length; q++) {
            if (MenuList[i].children[q].item.name == "餐具消毒") {
              for (
                var w = 0;
