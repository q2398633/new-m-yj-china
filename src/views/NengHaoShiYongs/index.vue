          ...
          <van-checkbox-group v-model="result" @change="CheckBoxChange">
            ...
            >
              ...
              <div class="ChildName" @click="Child_Countend($event)">
                <div>{{ Child.title }}</div>
                <div class="ChildName_Bottom">{{ Child.数量 }}</div>
                <div class="ChildName_Bottom2">{{ Child.jiLiangDanWei }}</div>
                <div class="ChildName_Bottom2">{{ Child.jinE }}元</div>
                <div class="ChildName_Bottom2">{{ Child.danJia }}元</div>
              </div>
            </van-cell>
          </van-checkbox-group>
        </div>
    ...
    >
      ...
      <div style="margin-top: 2.5rem">
        ...
        <van-form>
          ...
          />
          <van-field
            v-model="ChildListSearch.jiLiangDanWei"
            name="jiLiangDanWei"
            label="计量单位"
            placeholder="计量单位"
          />
          <van-field
            v-model="ChildListSearch.danJia"
            clearable
            name="danJia"
            label="单价"
            placeholder="单价"
          />
          <van-field
            v-model="ChildListSearch.数量"
            name="数量"
            label="数量"
            placeholder="数量"
          />
          <van-field
            v-model="ChildListSearch.jinE"
            name="jinE"
            label="金额"
            placeholder="金额"
          />
          <van-field
            v-model="ChildListSearch.yinHangZhangHuId"
            name="yinHangZhangHuId"
            label="银行账户"
            placeholder="银行账户"
          />
          <van-field
            v-model="ChildListSearch.yinHangZhangHuIdName"
...
import { NHSYList, NHSYDelete } from "@/api/NengHaoShiYongs";
...
export default {
  ...
  methods: {
    async onLoad() {
      ...
      this.LoadPage.page++;
      var that = this;
      const { data } = await NHSYList(this.LoadPage);
        ...
        for (var i in ChildList) {
          if (that.ChildName.length < ChildList.length) {
            that.ChildName.push({
              ...
              jiLiangDanWei: ChildList[i].jiLiangDanWei,
              date: ChildList[i].date,
              danJia: ChildList[i].danJia,
              数量: ChildList[i].数量,
              jinE: ChildList[i].jinE,
              yinHangZhangHuId: ChildList[i].yinHangZhangHuId,
              yinHangZhangHuIdName: ChildList[i].yinHangZhangHuIdName,
    ...
    async onSubmit(value) {
      // 筛选赋值
      this.see = false;
      const { data } = await NHSYList(this.ChildListSearch);
      const ChildList = data.data;
      for (var i = 0; i < ChildList.length; i++) {
        this.ChildName.push({
          title: ChildList[i].title + '',
          jiLiangDanWei: ChildList[i].jiLiangDanWei - 0,
          date: ChildList[i].date + '',
          danJia: ChildList[i].danJia - 0,
          数量: ChildList[i].数量 - 0,
          jinE: ChildList[i].jinE - 0,
          yinHangZhangHuId: ChildList[i].yinHangZhangHuId + '',
          yinHangZhangHuIdName: ChildList[i].yinHangZhangHuIdName + '',
          createTime: ChildList[i].createTime,
          createId: ChildList[i].createId,
    ...
    async tabClick(event) {
      ...
      if (ButtonText === "添加") {
        if (this.ckindex > 0) {
          ...
          this.$router.replace("/NengHaoShiYongsAdd");
        }
        this.CheckboxIndex = [];
      } else if (ButtonText === "修改") {
        if (this.ckindex <= 1) {
          this.$router.push({
            name: "NengHaoShiYongsEdit",
            params: {
              title: this.CheckboxID.title,
              jiLiangDanWei: this.CheckboxID.jiLiangDanWei,
              date: this.CheckboxID.date,
              danJia: this.CheckboxID.danJia,
              数量: this.CheckboxID.数量,
              jinE: this.CheckboxID.jinE,
              yinHangZhangHuId: this.CheckboxID.yinHangZhangHuId,
              yinHangZhangHuIdName: this.CheckboxID.yinHangZhangHuIdName,
        ...
        if (this.ckindex <= 1) {
          ...
          const { data } = await NHSYDelete(this.CID);
          if (data.code == 200) {
    ...
    Child_Countend(event) {
      for (var i in this.$refs.checkboxes) {
        if (
          event.target.innerText == this.$refs.checkboxes[i].name.xiaoDuFangFa
        ) {
          this.$router.push({
            name: "CanJuXiaoDusDetail",
      ...
      for (var i = 0; i < MenuList.length; i++) {
        if (MenuList[i].item.name == "食堂管理") {
          ...
          for (var q = 0; q < MenuList[i].children.length; q++) {
            if (MenuList[i].children[q].item.name == "能耗使用") {
              for (
                var w = 0;
