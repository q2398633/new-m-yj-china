          ...
          <van-checkbox-group v-model="result" @change="CheckBoxChange">
            ...
            >
              ...
              <div class="ChildName" @click="Child_Countend($event)">
                <div>{{ Child.title }}</div>
                <div class="ChildName_Bottom">{{ Child.jiLiangDanWei }}</div>
                <div class="ChildName_Bottom2">{{ Child.canKaoZhi }}</div>
                <div class="ChildName_Bottom2">{{ Child.beiZhu }}</div>
              </div>
            </van-cell>
          </van-checkbox-group>
        </div>
...
import { AQJCList, AQJCDelete } from "@/api/AnJianXiangMus";
...
export default {
  ...
  methods: {
    async onLoad() {
      ...
      this.LoadPage.page++;
      var that = this;
      const { data } = await AQJCList(this.LoadPage);
    ...
    async onSubmit() {
      // 筛选赋值
      this.see = false;
      const { data } = await AQJCList(JSON.stringify(this.ChildListSearch));
    ...
    async tabClick(event) {
      ...
      if (ButtonText === "添加") {
        if (this.ckindex > 0) {
          ...
          this.$router.replace("/AnJianXiangMusAdd");
        }
        this.CheckboxIndex = [];
      } else if (ButtonText === "修改") {
        if (this.ckindex <= 1) {
          this.$router.push({
            name: "AnJianXiangMusEdit",
            params: {
        ...
        if (this.ckindex <= 1) {
          ...
          const { data } = await AQJCDelete(this.CID);
          if (data.code == 200) {
      ...
      for (var i = 0; i < MenuList.length; i++) {
        if (MenuList[i].item.name == "食堂管理") {
          for (var q = 0; q < MenuList[i].children.length; q++) {
            if (MenuList[i].children[q].item.name == "安全检测项目") {
              for (
                var w = 0;
