          ...
          <van-checkbox-group v-model="result" @change="CheckBoxChange">
            ...
            >
              ...
              <div class="ChildName" @click="Child_Countend($event)">
                ...
                <div class="ChildName_Bottom2">{{ Child.jianCeJieGuo }}</div>
                <div class="ChildName_Bottom2">{{ Child.jianCeRenYuan }}</div>
                <div class="ChildName_Bottom2">{{ Child.jianCeRiQi }}</div>
              </div>
            </van-cell>
          </van-checkbox-group>
        </div>
    ...
    >
      ...
      <div style="margin-top: 2.5rem">
        ...
        <van-form @submit="onSubmit">
          ...
          />
          <van-field
            v-model="ChildListSearch.jianCeZhi"
            clearable
            name="jianCeZhi"
            label="检测值"
            placeholder="检测值"
          />
          <van-field
            v-model="ChildListSearch.canKaoZhi"
            name="canKaoZhi"
            label="参考值"
            placeholder="参考值"
          />
          <van-field
            v-model="ChildListSearch.jianCeJieGuo"
            name="jianCeJieGuo"
            label="检测结果"
            placeholder="检测结果"
          />
          ...
          />
          <van-field
            v-model="ChildListSearch.jianCeRiQi"
            name="jianCeRiQi"
            label="检测日期"
            placeholder="检测日期"
            @focus="noBomBox"
            :value-class="className"
            :value="ChildListSearch.jianCeRiQi"
...
import { SPJCList, SPJCDelete } from "@/api/ShiPinJianCes";
...
export default {
  ...
  methods: {
    async onLoad() {
      ...
      this.LoadPage.page++;
      var that = this;
      const { data } = await SPJCList(this.LoadPage);
        ...
        for (var i in ChildList) {
          if (that.ChildName.length < ChildList.length) {
            that.ChildName.push({
              ...
              canKaoZhi: ChildList[i].canKaoZhi,
              jianCeRenYuan: ChildList[i].jianCeRenYuan,
              jianCeRiQi: ChildList[i].jianCeRiQi,
              jianCeZhi: ChildList[i].jianCeZhi,
              jianCeJieGuo: ChildList[i].jianCeJieGuo,
    ...
    async onSubmit(values) {
      // 筛选赋值
      this.ChildListSearch.canKaoZhi =
        this.ChildListSearch.canKaoZhi - 0;
      this.ChildListSearch.jianCeZhi =
        this.ChildListSearch.jianCeZhi - 0;
      const { data } = await SPJCList(this.ChildListSearch);
    ...
    BirthdayConfirm(val) {
      ...
      this.LiuYangName = "日期";
      this.ChildListSearch.jianCeRiQi = `${year}-${month}-${day}`;
    ...
    async tabClick(event) {
      ...
      if (ButtonText === "添加") {
        if (this.ckindex > 0) {
          ...
          this.$router.replace("/ShiPinJianCesAdd");
        }
        this.CheckboxIndex = [];
      } else if (ButtonText === "修改") {
        if (this.ckindex <= 1) {
          this.$router.push({
            name: "ShiPinJianCesEdit",
            params: {
              title: this.CheckboxID.title,
              jiLiangDanWei: this.CheckboxID.jiLiangDanWei,
              canKaoZhi: this.CheckboxID.canKaoZhi,
              jianCeRenYuan: this.CheckboxID.jianCeRenYuan,
              jianCeRiQi: this.CheckboxID.jianCeRiQi,
              jianCeZhi: this.CheckboxID.jianCeZhi,
              jianCeJieGuo: this.CheckboxID.jianCeJieGuo,
        ...
        if (this.ckindex <= 1) {
          ...
          const { data } = await SPJCDelete(this.CID);
          if (data.code == 200) {
      ...
      for (var i = 0; i < MenuList.length; i++) {
        if (MenuList[i].item.name == "食堂管理") {
          ...
          for (var q = 0; q < MenuList[i].children.length; q++) {
            if (MenuList[i].children[q].item.name == "食品安全检测") {
              for (
                var w = 0;
