            ...
            >
              ...
              <div class="ChildName" @click="Child_Countend($event)">
                ...
                <div class="ChildName_Bottom2" style="margin-left: 0;">
                  单位:{{ Child.jiLiangDanWei }}
                </div>
                <div class="ChildName_Bottom2">单价:{{ Child.danJia }}元</div>
                <div class="ChildName_Bottom2">备注:{{ Child.beiZhu }}</div>
              </div>
            </van-cell>
          </van-checkbox-group>
        </div>
...
import { NHKZList, NHKZDelete } from "@/api/NengHaoXiangMus";
...
export default {
  ...
  methods: {
    async onLoad() {
      ...
      this.LoadPage.page++;
      var that = this;
      const { data } = await NHKZList(this.LoadPage);
    ...
    async onSubmit(values) {
      // 筛选赋值
      values.danJia = this.ChildListSearch.danJia - 0;
      values.jinE = this.ChildListSearch.jinE - 0;
      values.title = this.ChildListSearch.title;
      values.jiLiangDanWei = this.ChildListSearch.jiLiangDanWei;
      const { data } = await NHKZList(values);
    ...
    async tabClick(event) {
      ...
      if (ButtonText === "添加") {
        if (this.ckindex > 0) {
          ...
          this.$router.replace("/NengHaoXiangMusAdd");
        }
        this.CheckboxIndex = [];
      } else if (ButtonText === "修改") {
        if (this.ckindex <= 1) {
          this.$router.push({
            name: "NengHaoXiangMusEdit",
            params: {
        ...
        if (this.ckindex <= 1) {
          ...
          const { data } = await NHKZDelete(this.CID);
          if (data.code == 200) {
    ...
    Child_Countend(event) {
      for (var i in this.$refs.checkboxes) {
        if (
          event.target.innerText == this.$refs.checkboxes[i].name.title
        ) {
          this.$router.push({
            name: "NengHaoXiangMusDetail",
      ...
      for (var i = 0; i < MenuList.length; i++) {
        if (MenuList[i].item.name == "食堂管理") {
          ...
          for (var q = 0; q < MenuList[i].children.length; q++) {
            if (MenuList[i].children[q].item.name == "能耗控制项目") {
              for (
                var w = 0;
