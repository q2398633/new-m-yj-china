    ...
    >
      ...
      <div style="margin-top: 2.5rem">
        ...
        <van-form @submit="onSubmit">
          ...
          />
          <van-field
            v-model="ChildListSearch.xiaoDuQuYu"
            name="xiaoDuQuYu"
            label="消毒区域"
            placeholder="消毒区域"
          />
...
import { QYXDList, QYXDDelete } from "@/api/QuYuXiaoDus";
...
export default {
  ...
  methods: {
    async onLoad() {
      ...
      this.LoadPage.page++;
      var that = this;
      const { data } = await QYXDList(this.LoadPage);
      var ChildList = data.data;
      if (data.code == 200) {
        for (var i in ChildList) {
          if (that.ChildName.length < ChildList.length) {
            that.ChildName.push({
              xiaoDuQuYu: ChildList[i].xiaoDuQuYu,
    ...
    async onSubmit(values) {
      // 筛选赋值
      const { data } = await QYXDList(this.ChildListSearch);
    ...
    async tabClick(event) {
      ...
      if (ButtonText === "添加") {
        if (this.ckindex > 0) {
          ...
          this.$router.replace("/QuYuXiaoDusAdd");
        }
        this.CheckboxIndex = [];
      } else if (ButtonText === "修改") {
        if (this.ckindex <= 1) {
          this.$router.push({
            name: "QuYuXiaoDusEdit",
            params: {
              xiaoDuQuYu: this.CheckboxID.xiaoDuQuYu,
        ...
        if (this.ckindex <= 1) {
          ...
          const { data } = await QYXDDelete(this.CID);
          if (data.code == 200) {
    ...
    Child_Countend(event) {
      for (var i in this.$refs.checkboxes) {
        if (event.target.innerText == this.$refs.checkboxes[i].name.xiaoDuQuYu) {
          this.$router.push({
            name: "QuYuXiaoDusDetail",
      ...
      for (var i = 0; i < MenuList.length; i++) {
        if (MenuList[i].item.name == "食堂管理") {
          ...
          for (var q = 0; q < MenuList[i].children.length; q++) {
            if (MenuList[i].children[q].item.name == "区域消毒") {
              for (
                var w = 0;
