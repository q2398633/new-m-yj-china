<template>
  <div class="parentAdmin">
    ...
    <van-popup v-model="show"
               ...
               close-icon="close">
      <van-cell-group>
        <div style="font-size: 30px; width: 96.2%; height: 53px; line-height: 53px; color: white; font-family: '楷体'; background: #0199ff; padding-left:15px;">搜索产品信息</div>
        <van-field label="名称:"
                   label-width="70px"
                   autosize
                   name="G_Title_Like"
        ...
        <van-field label="供应商:"
                   label-width="70px"
                   autosize
                   name="G_GongYingShangIdName_Like"
                   prop="G_GongYingShangIdName_Like"
                   v-model="Search.G_GongYingShangIdName_Like"
                   placeholder="请输入单位" />
      </van-cell-group>
      <div class="submit">
          ...
          <van-cell v-for="(item) in list"
            ...
            <van-swipe-cell style="border: 7px solid rgb(231, 231, 231);">
              ...
              <van-cell :border="false"
                        title="供应商:"
                        style="padding-left:30px; padding-right: 30px;">
                {{ item.GongYingShangIdName }}- {{ item.LianXiRen }}
              </van-cell>
              <van-cell :border="false"
                        title="备注:"
                        style="padding-left:30px; padding-right: 30px;">
...
<script>
import { productManagement } from '@/api/productManagement'
...
export default {
    ...
    created () {
    // 页面一进入加载产品信息列表
        this.loadproductManagementList()
    },
    methods: {
        back () {
            this.$router.go(-1)
        ...
        async loadproductManagementList () {
            let channels = []
            const data = await productManagement()
        ...
        async onLoad () {
            const data = await this.loadproductManagementList()
            this.list = data
            this.isLoading = false
            this.loading = false
            this.finished = true
